            ...
            <Container>
                ...
                <ContentCover>
                    ...
                    <InnerCover className={'row'}>
                        {page > 0 ? (
                            <Control mode={PREV} onControl={handleControl} />
                        ) : (
                        ...
                        {savedStory.map(
                            (item, index) =>
                                item &&
                                page === index && <PreviewGeneratedImage index={index} pagelength={savedStory.length} />
                        )}
                        {page < savedStory.length - 1 ? (
                            <Control mode={NEXT} onControl={handleControl} />
                        ) : page === savedStory.length - 1 ? (
                            <Control mode={NEXT} onControl={handleControl} />
                        ) : null}
                        {page === savedStory.length && <PreviewAllGeneratedImage pagelength={savedStory.length} />}
                    </InnerCover>
                </ContentCover>
            </Container>
        </div>
