...
import { createStoryBook,recreate } from '../../service/FairytaleService';
...
const StoryGenerated = () => {
    ...
    const resendkeyword = useRecoilCallback(({ set }) => async (userDTO) => {
        try {
            const response = await recreate (userDTO);
            ...
            {loaded ? (
                <Container className={'an1'}>
                    ...
                    <ContentCover>
                        ...
                        <InnerCover>
                            <ButtonWrap>
                                <FontAwesomeIcon onClick={plusPageLength} icon={plus} size="2xl" />
                                <FontAwesomeIcon
